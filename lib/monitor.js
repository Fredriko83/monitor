          ...
          hosts.forEach((host) => {
            ping.promise
              ...
              .then((res) => {
                ...
                if (res.time === 'unknown') {
                  addTimeStamp(host, { active: false, responseTime: 0 });
...
function filterServerData(servers) {
  ...
  for (let i = 0; i < servers.length; i += 1) {
    const server = {
      ...
      lastPing: servers[i].timestamps[servers[i].timestamps.length - 1]
    };
    date = new Date();
    date.setMinutes(date.getMinutes() - 2);
    if(server.lastPing.date < date.getTime()){
      server.lastPing.active = false;
    }
